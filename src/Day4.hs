...
valid digits = isSorted digits && (any equalPair $ pairs digits)
...
newValid digits =
    let op = reverse $ countAdjacentPairs $ digits
    in
      not (length op == 1 && head op == 2) && isSorted op

-- TODO: 677788 Passes, when it shouldn't

day4part2 = print $ length
